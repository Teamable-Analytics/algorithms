...
class VariedNumEnemiesSocialRun(Run):
    @staticmethod
    def start(num_trials: int = 10, generate_graphs: bool = True):
        ...
        for class_size in class_sizes:
            ...
            for num_enemies in enemy_nums:
                print(
                    f"Running with {class_size} students with {num_enemies} enem{'ies' if num_enemies != 1 else 'y'}..."
                )
                ...
                simulation_sets[class_size][num_enemies] = SimulationSet(
                    settings=SimulationSettings(
                        num_teams=class_size // 5,
                        scenario=GiveThePeopleWhatTheyWant(),
                        student_provider=MockStudentProvider(student_provider_settings),
                        cache_key=f"social/varied_num_friends/{num_enemies}_enemies/{class_size}_students",
                    ),
