...
class ProportionalityUpToOneItem(TeamSetMetric):
    ...
    def calculate(self, team_set: TeamSet) -> float:
        ...
        return (
            self._is_prop(team_set, all_students)
            or self._is_prop_with_one_extra_student(team_set, all_students)
            or self._is_prop_with_one_less_student(team_set, all_students)
        )

    def _is_prop(self, team_set: TeamSet, all_students: List[Student]) -> bool:
        return all(
            [
                self._is_team_prop(team, all_students, team_set.num_teams)
                for team in team_set.teams
            ]
        )

    def _is_prop_with_one_extra_student(
        self, team_set: TeamSet, all_students: List[Student]
    ) -> bool:
        for team in team_set.teams:
            students_in_team = set([student.id for student in team.students])

            is_new_team_prop = all(
                [
                    self._is_team_prop(
                        Team(
                            _id=team.id,
            ...
            if not is_new_team_prop:
                return False

        return True

    def _is_prop_with_one_less_student(
        self, team_set: TeamSet, all_students: List[Student]
    ) -> bool:
        for team in team_set.teams:
            is_new_team_prop = all(
