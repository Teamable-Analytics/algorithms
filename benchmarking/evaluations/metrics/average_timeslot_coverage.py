...
class AverageTimeslotCoverage(TeamSetMetric):
    ...
    def __init__(self, available_timeslots: List[int],
    ...
    def calculate(self, team_set: TeamSet) -> float:
        ...
        for student in [student for team in team_set.teams for student in team.students]:
            if student.attributes.get(self.timeslot_attribute_value) is None:
                raise ValueError(f"Student {student.id} does not have the timeslot attribute")
            if any([attr not in self.available_timeslots for attr in student.attributes.get(self.timeslot_attribute_value)]):
        ...
        for team in team_set.teams:
            ...
            for timeslot in self.available_timeslots:
                if all(timeslot in student.attributes.get(self.timeslot_attribute_value) for student in team.students):
                    num_mutual_timeslots += 1
            timeslot_coverage += num_mutual_timeslots / float(len(self.available_timeslots))
