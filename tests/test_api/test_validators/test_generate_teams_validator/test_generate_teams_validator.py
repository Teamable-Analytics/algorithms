...
from validators.generate_teams_validator import GenerateTeamsValidator
...
class TestGenerateTeamsValidator(unittest.TestCase):
    ...
    def setUp(cls):
        ...
        student_with_invalid_relationship_data["students"][0]["relationships"] = {"-1": "enemy"}
        cls.student_with_invalid_relationship_data = student_with_invalid_relationship_data
        ...
        student_with_invalid_project_id_data["students"][0]["project_preferences"].append(-1)
        ...
        team_generation_options_with_duplicate_ids_data["team_generation_options"]["initial_teams"] += [
            { "id": 3, "project_id": 3 },
            { "id": 3, "project_id": 4 },
        ]
        cls.team_generation_options_with_duplicate_ids_data = team_generation_options_with_duplicate_ids_data
    ...
    def test_errors__top_levels(self):
        ...
        with self.assertRaises(SchemaError) as e:
            validator = GenerateTeamsValidator(self.missing_team_generation_options_data)
            validator.validate()
        self.assertEqual(e.exception.code, "Missing key: 'team_generation_options'")

    def test_errors__students(self):
        with self.assertRaises(SchemaError) as e:
            validator = GenerateTeamsValidator(self.student_with_invalid_relationship_data)
            validator.validate()
        self.assertEqual(e.exception.code, "Student 1 has relationships with unknown students.")

        with self.assertRaises(SchemaError) as e:
            validator = GenerateTeamsValidator(self.student_with_extra_attribute_data)
            validator.validate()
        self.assertEqual(e.exception.code, "Student 1 has attributes that do not exist.")

        with self.assertRaises(SchemaError) as e:
            validator = GenerateTeamsValidator(self.student_with_invalid_project_id_data)
            validator.validate()
        self.assertEqual(e.exception.code, "Student 1 has project preferences that do not exist in the project set.")

    def test_errors__team_generation_options(self):
        with self.assertRaises(SchemaError) as e:
            validator = GenerateTeamsValidator(self.team_generation_options_with_duplicate_ids_data)
            validator.validate()
