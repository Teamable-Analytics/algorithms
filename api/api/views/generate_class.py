        ...
        try:
            ...
            for key, value_dict in attribute_ranges_raw.items():
                ...
                if type(value_dict) is dict:
                    attribute_ranges[int(key)] = [(int(k), float(v)) for k, v in value_dict.items()]
                if type(value_dict) is list:
                    attribute_ranges[int(key)] = value_dict

            project_preference_options = list(map(int, request_data.get("project_preference_options", [])))
            ...
            student_provider_settings = MockStudentProviderSettings(
                ...
                num_project_preferences_per_student=int(request_data.get("num_project_preferences_per_student", 0)),
            ...
            json_student = [StudentSerializer().default(student) for student in students]
            return ResponseWithMetadata(data=json_student, data_label="students", status=200)
        except KeyError as e:
            return ResponseWithMetadata(error=f"Missing key {str(e)}", data_label="students", status=500)
