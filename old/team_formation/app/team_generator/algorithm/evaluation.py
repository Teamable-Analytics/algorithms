...
class TeamEvaluation:
    def __init__(self, teams: List[Team]):
        self.perfect_team_student_ids: List[int] = [
            student.id for student in self.get_perfect_team_students(teams)
        ]
        self.teams: List[Team] = teams
        self._students_cache: Dict[int, Student] = self.get_students_from_teams(
            self.teams
        )
    ...
    def is_satisfied(
        self, student_id: int, team_member_ids: List[int], friend: bool
    ) -> bool:
        """
    ...
    def is_reach_preference(
        ...
        if (
            self.opposite_pref(pref_giver.relationships[pref_target_id])
    ...
    def team_satisfaction(
        self, team: Team, friend: bool = True
    ...
    def team_set_satisfaction_metrics(
        self, friend: bool = True
    ) -> Tuple[int, int, int, int]:
        number_possible, number_satisfied, num_reach = 0, 0, 0
        for team in self.teams:
            _, team_possible, team_satisfied, reach = self.team_satisfaction(
                team, friend
            )
